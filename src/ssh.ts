...
import * as swcrypto from 'diffie-hellman';

const clientVersion = 'SSH-2.0-UNP_1.1';
const traceCrypto = false;
const traceSSHMessage = false;
...
const hexDump = function (a: Buffer, offset?: number, length?: number) {
  const start = (offset ? offset : 0);
  const len = (length ? length : a.length);
  let buff = "";
  for (let i = 0; i < len; i++) {
    buff += hex(a[start + i]) + " ";
    if ((i % 16) == 15) {
...
const SSH_MESSAGE: { [message: string]: number } = {
...
export const MESSAGE = SSH_MESSAGE;

const CODE_TO_NAME: { [code: number]: string } = {};
...
const maxPacketSize = 0x10000; // lower the size from 2*0x10000 as stack may overflow

const initialWindowSize = maxPacketSize * 64; // 0x100000*2
const SSH_CONNECTION = 'ssh-connection';
const KEYBOARD_INTERACTIVE = 'keyboard-interactive';
const Min_N_Max = [1024, 4096, 8192];

const SSH_TO_OPENSSL_TABLE: { [alg: string]: string } = {
...
const SUPPORTED_COMPRESS = [
  'none'
];


export const setLogger = (logger: any): void => {
  sshLogger = logger;
}

export const processEncryptedData = function (terminalWebsocketProxy: any, data: any) {
  return sshProcessEncryptedData(terminalWebsocketProxy, data);
};

export const sendSSHData = function (terminalWebsocketProxy: any, data: any) {
  return sshSendSSHData(terminalWebsocketProxy, data);
};

const sshSendSSHData = function (terminalWebsocketProxy: any, jsonData: any) {
  const clientIP = terminalWebsocketProxy.clientIP;

  const sessionData = terminalWebsocketProxy.sshSessionData;
  if (!sessionData) {
    return; // error thrown
  }
  const msgCode = jsonData.msgCode;
  let pdu;
  ...
  switch (msgCode) {
    case SSH_MESSAGE.SSH_MSG_USERAUTH_INFO_RESPONSE: {
      ...
      if (jsonData.responses) {
        ...
        const numResponses = jsonData.responses.length;
        const data = [msgCodeBuffer, jsonData.responses.length];
        jsonData.responses.forEach(function (response: string) {
    ...
    case SSH_MESSAGE.SSH_MSG_USERAUTH_REQUEST: {
      if (jsonData.method === 'password') {
        ...
        const username = new VarData(Buffer.from(jsonData.username, 'utf8'));
        const sshconnection = new VarData(Buffer.from(SSH_CONNECTION, 'utf8'));
        const passwordMethod = new VarData(Buffer.from('password', 'utf8'));//this could be publickey,hostbased, or none. USERAUTH_FAILURE results when this attempt was no good.
        //USERAUTH_SUCCESS means the server approved.
        const authData = [msgCodeBuffer, username, sshconnection, passwordMethod, new Boolean(false), new VarData(Buffer.from(jsonData.password, 'utf8'))];
    ...
    case SSH_MESSAGE.SSH_MSG_CHANNEL_DATA: {
      // sending data
      const dataBuffer = jsonData.data;
      const channelNumber = sessionData.primaryShellChannel;
      const channel = sessionData.channels[channelNumber];
      if (channel) {
        const msgCodeBuffer = Buffer.alloc(1);
        msgCodeBuffer.writeUInt8(SSH_MESSAGE.SSH_MSG_CHANNEL_DATA);
        const shellData = [msgCodeBuffer, new Number(channel.serverChannelNumber), new VarData(dataBuffer)];
    ...
    case SSH_MESSAGE.SSH_MSG_CHANNEL_REQUEST: {
      ...
      if (channel) {
        const requestData = [msgCodeBuffer, jsonData.channel ? new Number(jsonData.channel) : new Number(channel.serverChannelNumber), new VarData(Buffer.from(jsonData.type)), new Boolean(jsonData.reply)];
        for (let i = 0; i < jsonData.requestContents.length; i++) {
  ...
  if (pdu != null) {
    if (sessionData.isKeyExchanging) {
      ...
      let queuedPdu = sessionData.rekeyQueue.shift();
      while (queuedPdu) {
...
// main method. 
const sshProcessEncryptedData = function (terminalWebsocketProxy: any, rawData: Buffer) {
  const clientIP = terminalWebsocketProxy.clientIP;
  const sshMessages: any[] = [];

  const returnError = function (msg: string) {
    return [{ type: SSH_MESSAGE.ERROR, msg: msg }];
  };

  let sessionData = terminalWebsocketProxy.sshSessionData;
  if (!sessionData) {
    sessionData = new SSHSessionData();
    const serverVersion = new String(rawData).trim();
    sessionData.serverVersion = Buffer.from(serverVersion, 'utf8');

    const dataBuffer = Buffer.from(clientVersion + '\r\n');
  ...
  const channelNumber = sessionData.primaryShellChannel == null ? sessionData.highestChannel : sessionData.primaryShellChannel;
  ...
  while (currentPosition < rawDataLength) {
    ...
    const readObject = readSSHv2PDUData(sessionData, rawData, currentPosition, clientIP); // JERRY: Payload is stored in readObject.sshv2PDU.data
    ...
    const sshv2PDU = readObject.sshv2PDU;
    currentPosition = readObject.readLength;
    sshLogger.log(sshLogger.FINER, `[SSH, ClientIP=${clientIP}, MsgSeq: ${sessionData.messagesReceived}] - `
      + `Raw data length: ${rawDataLength}, position after read: ${currentPosition}`);
    const msgCode = sshv2PDU.readByte() & 0xff;  // JERRY: read a byte from Payload
    const sshMessage: any = {};
    if (msgCode) {
    ...
    switch (msgCode) {
      ...
      case SSH_MESSAGE.SSH_MSG_USERAUTH_SUCCESS: {
        ...
        const sessionVar = new VarData(Buffer.from('session', 'utf8'));
        const openChannelRequestData = [msgCodeBuffer, sessionVar, new Number(channelNumber), new Number(initialWindowSize), new Number(maxPacketSize)];

        const openChannelRequestPDU = new SSHv2PDU(SSH_MESSAGE.SSH_MSG_CHANNEL_OPEN, generateSSHv2PDUBytes(openChannelRequestData));
        writeSSHv2PDU(function (buffer: Buffer) { terminalWebsocketProxy.netSend(buffer); }, sessionData, openChannelRequestPDU);
      ...
      case SSH_MESSAGE.SSH_MSG_CHANNEL_OPEN_CONFIRMATION: {
        ...
        const peerChannelNumber = sshv2PDU.readInt();
        const peerWindowSize = sshv2PDU.readInt();
        const peerMaxPacketSize = sshv2PDU.readInt();
        ...
        const msgCodeBuffer = Buffer.alloc(1);
        msgCodeBuffer.writeUInt8(SSH_MESSAGE.SSH_MSG_CHANNEL_REQUEST);
        const terMode = new VarData(Buffer.from([TTY_OP_END]));//add CS7=90 maybe?

        const settings = terminalWebsocketProxy.securitySettings;
        ...
        const shellPTYRequestData = [msgCodeBuffer, new Number(peerChannelNumber), new VarData(Buffer.from('pty-req')),
        ...
        const shellPTYRequestPDU = new SSHv2PDU(SSH_MESSAGE.SSH_MSG_CHANNEL_REQUEST, generateSSHv2PDUBytes(shellPTYRequestData));
        writeSSHv2PDU(function (buffer: Buffer) { terminalWebsocketProxy.netSend(buffer); }, sessionData, shellPTYRequestPDU);
      ...
      case SSH_MESSAGE.SSH_MSG_CHANNEL_DATA: {
        ...
        const recipientChannel = sshv2PDU.readInt();
      ...
      case SSH_MESSAGE.SSH_MSG_CHANNEL_SUCCESS: {
        if (sessionData.channels[channelNumber]) {
          const currentChannel = sessionData.channels[channelNumber];
          if (!currentChannel.secondShellRequest) {
            currentChannel.secondShellRequest = true;
            const msgCodeBuffer = Buffer.alloc(1);
            msgCodeBuffer.writeUInt8(SSH_MESSAGE.SSH_MSG_CHANNEL_REQUEST);
            const terMode = new VarData(Buffer.from([TTY_OP_END]));
            const shellPTYRequestData = [msgCodeBuffer, new Number(currentChannel.peerChannelNumber), new VarData(Buffer.from('shell')), new Boolean(true)];
            const shellPTYRequestPDU = new SSHv2PDU(SSH_MESSAGE.SSH_MSG_CHANNEL_REQUEST, generateSSHv2PDUBytes(shellPTYRequestData));
      ...
      case SSH_MESSAGE.SSH_MSG_KEXINIT: {
        ...
        const keyExchangeAlgorithms = sshv2PDU.readNameList();
        const serverHostKeyAlgorithms = sshv2PDU.readNameList();

        const clientToServerEncryptionAlgorithms = sshv2PDU.readNameList();
        const serverToClientEncryptionAlgorithms = sshv2PDU.readNameList();

        const clientToServerMACAlgorithms = sshv2PDU.readNameList();
        const serverToClientMACAlgorithms = sshv2PDU.readNameList();

        const clientToServerCompressionAlgorithms = sshv2PDU.readNameList();
        const serverToClientCompressionAlgorithms = sshv2PDU.readNameList();

        const clientToServerLanguages = sshv2PDU.readNameList();
        const serverToClientLanguages = sshv2PDU.readNameList();

        const serverFirstKexPacketFollows = sshv2PDU.readByte();
        if (traceCrypto) console.log("serverFirstKexPacketFollows " + serverFirstKexPacketFollows);
        const reservedParam = sshv2PDU.readInt();
        sessionData.keyExchangePayload = sshv2PDU.data;

        const clientKexPDU = makeKeyExchangeResponse(sessionData, keyExchangeAlgorithms,
        ...
        switch (sessionData.keyExchangeAlgorithms[0]) {
          ...
          case 'diffie-hellman-group-exchange-sha1': {
            ...
            const argsArray = [msgCodeBuffer, Min_N_Max[0], Min_N_Max[1], Min_N_Max[2]];
            const dhGroupRequestPDU = new SSHv2PDU(SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_REQUEST, generateSSHv2PDUBytes(argsArray));
            sessionData.expectedReplyMsgCode = SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_GROUP;
        ...
        if (dh) {
          dh.generateKeys();
          let publicKeys = dh.getPublicKey();
          ...
          */
          ...
          if (publicKeys[idx] & 0x80) {
            const tempkey = Buffer.alloc(len + 1);
            tempkey[0] = 0;
          ...
          const keyLength = new Number(publicKeys.length);
          const kexInitPDUData = [msgCodeBuffer, keyLength, publicKeys];
          const kexInitPDU = new SSHv2PDU(SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_REQUEST_OLD, generateSSHv2PDUBytes(kexInitPDUData));
          writeSSHv2PDU(function (buffer: Buffer) { terminalWebsocketProxy.netSend(buffer); }, sessionData, kexInitPDU);
          sessionData.dh = dh;
      ...
      case SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_GROUP: {
        ...
        if (sessionData.sentMsgCode == SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_REQUEST) {
          ...
          const dh = swcrypto.createDiffieHellman(sessionData.prime, sessionData.generator)
          ...
          const keyLength = new Number(publicKeys.length);
          const kexInitPDUData = [msgCodeBuffer, keyLength, publicKeys];
          const kexInitPDU = new SSHv2PDU(SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_INIT, generateSSHv2PDUBytes(kexInitPDUData));
          writeSSHv2PDU(function (buffer: Buffer) { terminalWebsocketProxy.netSend(buffer); }, sessionData, kexInitPDU);
          sessionData.dh = dh;
          break;
        } else {
      ...
      case SSH_MESSAGE.SSH_MSG_KEX_DH_GEX_REPLY: {
        ...
        const hostKeyAndCertificates = sshv2PDU.readSizedData();
        const hostKeyPseudoPDU = new SSHv2PDU(-1, hostKeyAndCertificates);

        if (traceCrypto) console.log("hostKeyAndCertificates:");

        if (traceCrypto) console.log(hostKeyAndCertificates.toString('hex') + "\n");

        const serverHostKeyAlgorithm = hostKeyPseudoPDU.readSizedData();
        ...
        const signature = sshv2PDU.readSizedData();

        if (traceCrypto) console.log("signature:");

        if (traceCrypto) console.log(signature.toString('hex') + "\n");

        const signaturePDU = new SSHv2PDU(-1, signature);
        const sigFormat = signaturePDU.readSizedData();


        if (sessionData.dh) {
          const sharedSecret = sessionData.dh.computeSecret(sessionData.f);
          if (sharedSecret) {
        ...
        // compute H 
        const serverVersionLength = Buffer.byteLength(sessionData.serverVersion);
        const clientVersionLength = Buffer.byteLength(clientVersion);
        const clientKexLength = sessionData.clientKexPayload.length;
        const serverKexLength = sessionData.keyExchangePayload.length;
        const hostKeyLength = sessionData.hostKeyPseudoPayload.length;
        let clientPubKeyLength = sessionData.e.length;
        let serverPubKeyLength = sessionData.f.length;
        let secretlength = sessionData.k.length;
        ...
        let exchangeBufferLength = serverVersionLength
          + clientVersionLength
          + clientKexLength
          + serverKexLength
          + hostKeyLength
        ...
        let primeLength = 0;
        let generatorLength = 0;
        let sessionPrime: any;
        let sessionGenerator: any;
        let primeIndex = 0;
        ...
        let cp = 0;
        const exchangeDataBuffer = Buffer.alloc(exchangeBufferLength);
      ...
      case SSH_MESSAGE.SSH_MSG_NEWKEYS: {
        ...
        const clientNewKeyPDU = new SSHv2PDU(SSH_MESSAGE.SSH_MSG_NEWKEYS, generateSSHv2PDUBytes([msgCodeBuffer]));
        writeSSHv2PDU(function (buffer: Buffer) { terminalWebsocketProxy.netSend(buffer); }, sessionData, clientNewKeyPDU);
        ...
        if (sessionData.isKeyExchanging) {
          ...
          let pdu = sessionData.rekeyQueue.shift();
          while (pdu) {
            writeSSHv2PDU(function (buffer: Buffer) { terminalWebsocketProxy.netSend(buffer); }, sessionData, pdu);
        ...
        if (!terminalWebsocketProxy.sshAuthenticated) {
          ...
          const sshAuth = Buffer.from('ssh-userauth');
          const authReqLength = sshAuth.length;
          const userauthRequestPDU = new SSHv2PDU(SSH_MESSAGE.SSH_MSG_SERVICE_REQUEST, generateSSHv2PDUBytes([msgCodeBuffer, new Number(authReqLength), sshAuth]));

          writeSSHv2PDU(function (buffer: Buffer) { terminalWebsocketProxy.netSend(buffer); }, sessionData, userauthRequestPDU);
...
class SSHSessionData {
  ...
  clientCookie: any;
  serverVersion: any;
  incompletePacketBuffer: any;
  isFirstBlockDecrypted: any;
  sshMessageCode: any;
  isKeyExchanging: boolean;
  isKeyExchangingInitSent: boolean;
  rekeyQueue: any[];
  h: any;
  e: any;
  f: any;
  ...
  hash: any;
  hashAlgorithem: any;
  expectedReplyMsgCode: any;
  sentMsgCode: any;
  keyExchangePayload: any;
  clientKexPayload: any;
  hostKeyPseudoPayload: any;
  keyExchangeAlgorithms: any;
  serverHostKeyAlgorithms: any;
  clientToServerEncryptionAlgorithms: any;
  serverToClientEncryptionAlgorithms: any;
  clientToServerMACAlgorithms: any;
  serverToClientMACAlgorithms: any;
  clientToServerCompressionAlgorithms: any;
  serverToClientCompressionAlgorithms: any;
  id: any;
  readCipherBlockLength: number;
  writeCipherBlockLength: number;
  decryptCipher: any;
  encryptCipher: any;
  readingMACLength: number;
  writingMACLength: number;
  writingMACAlgorithm: any;
  readingMACAlgorithm: any;
  writingMACKey: any;
  readingMACKey: any;
  messagesSent: number;
...
class SSHv2PDU {
  ...
  constructor(messageCode: number, payload: Buffer) {
  ...
  readInt() {
    const currentOffset = this.offset;
    const payload = this.data;
    const res = payload.readUInt32BE(currentOffset);
    this.offset += 4;
    return res;
  }
  ...
  getRemainingBytes() {
    const currentOffset = this.offset;
    const payload = this.data;
    const remaining = Buffer.alloc(payload.length - currentOffset);
  ...
  readSizedData() {
    const currentOffset = this.offset;
    const payload = this.data;
    const length = payload.readUInt32BE(currentOffset);
  ...
  readNameList() {
    const payload = this.data;
    const currentOffset = this.offset;
    const size = payload.readUInt32BE(currentOffset);
    const kexNames = Buffer.alloc(size);
    payload.copy(kexNames, 0, currentOffset + 4);

    const commaSeparatedNames = kexNames.toString('ascii');
    const res = commaSeparatedNames.split(',');
    this.offset += (size + 4);
    return res;
  }
...
class SSH2ChannelData {
  clientChannelNumber: number;
  serverChannelNumber: number;
  clientWindowSize: number;
  clientMaxPacketSize: number;
  serverWindowSize: number;
  serverMaxPacketSize: number;
  secondShellRequest: boolean;


  constructor(channelNumber: number,
    peerChannelNumber: number,
    initialWindowSize: number,
    maxPacketSize: number, peerWindowSize: number,
    peerMaxPacketSize: number,
    isSecondShellRequest: boolean
  ) {
...
function readSSHv2PDUData(sessionData: any, rawData: any, currentPosition: number, clientIP: string) {

  let computedMac: crypto.Hmac | Buffer;
  const readCipherBlockLength = sessionData.readCipherBlockLength;
  let decryptedData;
  const messageSequenceNumber = sessionData.messagesReceived; // needed for MAC computation
  ...
  if (sessionData.decryptCipher != null) {
    ...
    const maxSize = maxPacketSize + 4 + sessionData.readingMACLength + 50;
    ...
    const remainingPacketLengthInThisRawDataLength = rawData.length - currentPosition;
    ...
    const readDataLength = packetLength - bytesRead;//because we already read length and some bytes more, depend on readCipherBlockLength

    const encryptedBytesRead = 0;
    const encryptedData = Buffer.alloc(readDataLength);
    ...
    const remainingData = sessionData.decryptCipher.update(encryptedData);
    ...
    if (sessionData.readingMACAlgorithm != null) {
      ...
      const seqNumberBuffer = Buffer.alloc(4);
    ...
    const decryptedDataPaddedLength = Buffer.alloc(currentPosition);
  ...
  const packetLength = decryptedData.readUInt32BE(currentPosition);  // JERRY: useful in case the raw data is decrypted
  currentPosition += 4;
  ...
  const paddingLength = decryptedData.readUInt8(currentPosition) & 0xff;
  currentPosition += 1;
  const payloadLength = packetLength - (paddingLength + 1); // +1 for the padding length byte
  ...
  if (sessionData.readingMACAlgorithm) {
    ...
    const receivedMac = Buffer.alloc(sessionData.readingMACLength);
...
function writeSSHv2PDU(socketWriteFunction: (buffer: Buffer) => void, sessionData: any, pdu: any) {

  const payload = pdu.data;

  const messageSequenceNumber = sessionData.messagesSent; // needed for MAC computation
  ...
  const lengthWithoutPadding = payload.length + 5;
  const paddingModulus = Math.max(sessionData.writeCipherBlockLength, 8);
  let paddingLength = paddingModulus - (lengthWithoutPadding % paddingModulus);
  if (paddingLength < 4) {
  ...
  const data = [];
  let packetBuffer = Buffer.alloc(payload.length + paddingLength + 5);

  // if (sessionData.writingMACAlgorithm != null || sessionData.encryptCipher != null) {// implies encryption too
  //}

  let packetBufferOffset = packetBuffer.writeInt32BE(payload.length + paddingLength + 1);
...
function initializeCrypto(sessionData: any) {

  const kData = sessionData.k;
  let bufPointer = 0;
  let index = 0;
  let sharedSecretlen = kData.length;
  while (kData[index] === 0x00) {
    ++index;
    --sharedSecretlen;
  }
  const secretlen = (kData[index] & 0x80 ? 1 : 0) + sharedSecretlen;
  const secretData = Buffer.alloc(4 + secretlen);
  ...
  const hashAlgorithm = sessionData.hashAlgorithem;
  const h = sessionData.h;
  ...
  const clientToServerEncryptionAlgorithm = sessionData.clientToServerEncryptionAlgorithms[0];
  const serverToClientEncryptionAlgorithm = sessionData.serverToClientEncryptionAlgorithms[0];
  const clientToServerMACAlgorithm = sessionData.clientToServerMACAlgorithms[0];
  const serverToClientMACAlgorithm = sessionData.serverToClientMACAlgorithms[0];
  ...
  const clientToServerIVBlock = generateKeyBlock(hashAlgorithm, secretData, h, 'A', sessionID, blockLength);
  ...
  const clientToServerEncryptionKeyBlock = generateKeyBlock(hashAlgorithm, secretData, h, 'C', sessionID, encryptionKeyLength);
  if (traceCrypto) {
  ...
  const cipherAlgo = SSH_TO_OPENSSL_TABLE[clientToServerEncryptionAlgorithm];
  ...
  const serverToClientIVBlock = generateKeyBlock(hashAlgorithm, secretData, h, 'B', sessionID, blockLength);
  ...
  const serverToClientEncryptionKeyBlock = generateKeyBlock(hashAlgorithm, secretData, h, 'D', sessionID, encryptionKeyLength);
  ...
  const decipherAlgo = SSH_TO_OPENSSL_TABLE[serverToClientEncryptionAlgorithm];
  ...
  const clientToServerMACBlock = generateKeyBlock(hashAlgorithm, secretData, h, 'E', sessionID, createKeyLen);
  const serverToClientMACBlock = generateKeyBlock(hashAlgorithm, secretData, h, 'F', sessionID, checkKeyLen);
...
function releaseImtermediateSessionData(sessionData: any) {
...
function generateKeyBlock(hashAlgorithm: string, secretData: Buffer, h: any, letter: string, sessionID: any, blockLength: number) {
  let dataBlock = crypto.createHash(hashAlgorithm)
    .update(secretData)
    .update(h)
    .update(letter, 'ascii')
    .update(sessionID)
...
function getAgreeableAlgorithms(clientSupportedAlgorithms: any, serverSupportedAlgorithms: any) {
  if (serverSupportedAlgorithms == null) {
    return clientSupportedAlgorithms;
  }
  const agreeableAlgorithems = [];
  for (let i = 0, len = clientSupportedAlgorithms.length; i < len; ++i) {
    const candidate = clientSupportedAlgorithms[i];
...
function makeKeyExchangeResponse(sessionData: any, keyExchangeAlgorithms: string[],
  serverHostKeyAlgorithms: string[],
  clientToServerEncryptionAlgorithms: string[],
  serverToClientEncryptionAlgorithms: string[],
  clientToServerMACAlgorithms: string[],
  serverToClientMACAlgorithms: string[],
  clientToServerCompressionAlgorithms: string[],
  serverToClientCompressionAlgorithms: string[]) {
  const agreeableKeyExchangeAlgorithms = getAgreeableAlgorithms(SUPPORTED_KEX, keyExchangeAlgorithms);
  ...
  const agreeableServerHostKeyAlgorithms = getAgreeableAlgorithms(SUPPORTED_SERVER_HOST_KEY, serverHostKeyAlgorithms);
  ...
  const agreeableClientToServerEncryptionAlgorithms = getAgreeableAlgorithms(SUPPORTED_CIPHER, clientToServerEncryptionAlgorithms);
  ...
  const agreeableServerToClientEncryptionAlgorithms = getAgreeableAlgorithms(SUPPORTED_CIPHER, serverToClientEncryptionAlgorithms);
  ...
  const agreeableClientToServerMACAlgorithms = getAgreeableAlgorithms(SUPPORTED_HMAC, clientToServerMACAlgorithms);
  ...
  const agreeableServerToClientMACAlgorithms = getAgreeableAlgorithms(SUPPORTED_HMAC, serverToClientMACAlgorithms);
  ...
  const agreeableClientToServerCompressionAlgorithms = getAgreeableAlgorithms(SUPPORTED_COMPRESS, clientToServerCompressionAlgorithms);
  ...
  const agreeableServerToClientCompressionAlgorithms = getAgreeableAlgorithms(SUPPORTED_COMPRESS, serverToClientCompressionAlgorithms);
  ...
  const clientCookie = crypto.randomBytes(16);
  ...
  const payload = generateSSHv2PDUBytes(payloadArray);
  return new SSHv2PDU(SSH_MESSAGE.SSH_MSG_KEXINIT, payload);

}


function generateSSHv2PDUBytes(argsArray: any[]) {
  const bufferArray = new Array<any>();

  for (let i = 0; i < argsArray.length; i++) {
    const object = argsArray[i];
    let objectBuffer: Buffer;

    if (object instanceof Number || (typeof object == 'number')) {
      //four bytes
      const objectBuffer = Buffer.alloc(4);
      objectBuffer.writeInt32BE(object as number);

    } else if (Array.isArray(object)) {
      const stringList = new Array<Buffer>();
      for (let j = 0; j < object.length; j++) {
        const string = object[j];
        if (j > 0) {
          stringList.push(Buffer.from(','));
        }
        stringList.push(Buffer.from(string));
      }
      const concatedStringBuffer = Buffer.concat(stringList);
      ...
      const objectBuffer = Buffer.alloc(1);
      objectBuffer.writeUInt8(Number(object));
    } else if (object instanceof VarData) {
      const varLength = object.data ? object.data.length : 0;
      const objectBuffer = Buffer.alloc(4 + varLength);
      objectBuffer.writeInt32BE(varLength);
      if (varLength) { object.data.copy(objectBuffer, 4); }
    } else if (typeof object == 'string') {
      const varLength = object ? object.length : 0;
      const objectBuffer = Buffer.alloc(4 + varLength);
      objectBuffer.writeInt32BE(varLength);
